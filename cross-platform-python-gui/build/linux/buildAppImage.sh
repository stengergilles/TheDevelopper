...
wget -O /tmp/python3.9.AppImage https://github.com/niess/python-appimage/releases/download/python3.9/python3.9.12-cp39-cp39-manylinux2014_x86_64.AppImage
chmod +x /tmp/python3.9.AppImage
/tmp/python3.9.AppImage --appimage-extract
mv squashfs-root /tmp/kivy_appdir

# copy depends that were installed with kivy into our kivy AppDir
rsync -a /tmp/kivy_venv/ /tmp/kivy_appdir/opt/python3.9/

# add our code to the AppDir
rsync -a ../*.py ../*.png /tmp/kivy_appdir/opt/
...
do
    ...
    if [[ "${opt}" =~ "I" ]] || [[ "${opt}" =~ "E" ]]; then
        ...
        "$APPDIR/opt/python3.9/bin/python3.9 $APPDIR/opt/src/main.py" "$@"
        exit "$?"
    fi
done
...
# extracted image
executable="${APPDIR}/opt/python3.9/bin/python3.9 ${APPDIR}/opt/src/main.py"
if [[ "${ARGV0}" =~ "/" ]]; then
...
# install ($PYTHONHOME)
(PYTHONHOME="${APPDIR}/opt/python3.9" exec -a "${executable}" "$APPDIR/opt/python3.9/bin/python3.9" "$APPDIR/opt/src/main.py" "$@")
exit "$?"
EOF
...
mkdir dist
/tmp/appimagetool.AppImage /tmp/kivy_appdir dist/TheDevelopper.AppImage
